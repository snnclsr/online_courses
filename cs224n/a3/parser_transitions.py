...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition is "S" and len(self.buffer) > 0:
            left_most_word = self.buffer.pop(0)
            self.stack.append(left_most_word)

        elif transition is "LA" and len(self.stack) > 1:
            root_word = self.stack[-2]
            dependency = (self.stack[-1], root_word)
            _ = self.stack.pop(-2)
            self.dependencies.append(dependency)
        else:
            if len(self.stack) > 1:
                root_word = self.stack[-1]
                dependency = (self.stack[-2], root_word)
                _ = self.stack.pop(-1)
                self.dependencies.append(dependency)
